...
optList <- list(
                ...
                make_option("--alpha", default = 1e-8, action = "store", help ="LOH alpha (default: %default)"))
...
loh = multi.LOH.analyze(normal = normal, tumor = tumor, all.loh.ls = list(eLOH), test.alpha = opt$alpha, method = opt$cbsLohMethod, sdundo = c(2,2), alpha = c(0.000001,0.000001))
